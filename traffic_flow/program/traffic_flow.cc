...
struct Traffic {
	...
	void operator()(const Doub x, VecDoub_I &y, VecDoub_O &dydx) {
		...
		//
		//Establish peiordic boundary conditions.
		//	This is the derivative associated
		//	with a viscosity value visc.
		//
	...
	TrafficIntegrator(double rho_bar_in, double del_rho_in, 
		...
		if (visc != 0.0) {
			filename << "visc=" << visc << "_rho_bar=" << rho_bar_in << 
						"_t=" << int(10.0*t_final) << ".dat" << ends;
		} else {
			filename << rho_bar_in << "_data_t=" << 
						int(10.0*t_final) << ".dat" << ends;	
		}	
	}
	~TrafficIntegrator() {
		delete ystart;
